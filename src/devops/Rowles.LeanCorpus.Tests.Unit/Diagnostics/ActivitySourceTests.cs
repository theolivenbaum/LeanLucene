...
/// <summary>
/// Verifies that <see cref="LeanCorpusActivitySource"/> emits activities with the expected names
...
{
    private const string SourceName = "Rowles.LeanCorpus";
    private const string TestSourceName = "Rowles.LeanCorpus.Tests.ActivityScope";
    ...
    public void Commit_EmitsActivity_WithSegmentCountTag()
    {
        ...
        var activity = Scoped(scope).FirstOrDefault(a => a.OperationName == "leancorpus.index.commit");
        Assert.NotNull(activity);
    ...
    public void Flush_EmitsActivity_WithDocCountTag()
    {
        ...
        var activity = Scoped(scope).FirstOrDefault(a => a.OperationName == "leancorpus.index.flush");
        Assert.NotNull(activity);
    ...
    public void NoListener_ProducesNoActivities()
    {
        ...
        Assert.DoesNotContain(_captured, a => a.OperationName == "leancorpus.index.flush");
    }
